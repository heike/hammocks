...
cols <- c(brewer.pal(5,"Blues")[-1], rev(brewer.pal(3, "Oranges")[-1]), rev(brewer.pal(3, "Greens")[-1]))
...
ggparallel(names(titanic)[c(1,4,2,1)], order=c(0,1,1,0), titanic, weight="Freq", method = 'hammock', 
  ...
  scale_y_continuous(breaks = seq(from  = 0, to = 2000, by =250))

ggsave(filename = 'titanic_hammock.png', width= 5, height = 5, units = 'in')
...
ggsave(filename = 'titanic_ca.png', width= 5, height = 5, units = 'in')
...
ggsave(filename = 'titanic_aa.png', width= 5, height = 5, units = 'in')

ggparallel(names(titanic)[c(1,4,2,1)], order=c(0,1,1,0), asp.ratio =.6, method="parset", titanic, weight="Freq") +
  scale_fill_manual(values=cols, guide="none") +
  scale_colour_manual(values=cols, guide="none") + theme_bw() +
  scale_y_continuous(breaks = seq(from  = 0, to = 2000, by =250))

ggsave(filename = 'titanic_parset.png', width= 5, height = 5, units = 'in')
